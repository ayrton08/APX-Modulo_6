...
(function() {
    _state.state.init();
    _state.state.setEmailAndName({
        email: "ayrtonjuarez90@gmail.com",
        fullName: "Ayrton"
    });
    _state.state.signIn((err)=>{
        if (err) console.error("Hubo unn error");
...
const state = {
    ...
    //     });
    // },
    setEmailAndName (params) {
        const currentState = this.getState();
        currentState.fullName = params.fullName, currentState.email = params.email;
        this.setState(currentState);
    },
    signIn (callback) {
        const currentState = this.getState();
        if (currentState.email) return fetch(API_BASE_URL + "/auth", {
            method: "post",
            headers: {
                "Content-Type": "application/json"
            },
    ...
    accessToRoom (roomId, callback) {
        const currentState = this.getState();
        fetch(API_BASE_URL + "/rooms/" + roomId + "?userId" + currentState.userId, {}).then((res)=>{
            return res.json();
        }).then((data)=>{
...
},{}]},["7mgxS","h7u1C"], "h7u1C", "parcelRequired883")
